...
impl Database {
    ...
    pub fn save_meta(&self) -> std::io::Result<()>{
        ...
        let meta_contents = serde_json::to_string(&self.meta).expect("Serializing meta state failed. ");
        file.write_all(meta_contents.as_bytes()).expect("Could not write to metadata file. ");
        Ok(())
    }
